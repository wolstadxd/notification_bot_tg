...
async def add_user_start(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text("📝 Введіть Telegram ID користувача, якого потрібно додати:")
...
async def process_add_user(message: Message, state: FSMContext):
    ...
    try:
        ...
        if user_id in current_users:
            await message.answer(f"⚠️ Користувач `{user_id}` вже є в списку дозволених.", parse_mode="Markdown")
            await state.clear()
            return
        ...
        await message.answer(f"✅ Користувач `{user_id}` успішно додано до списку дозволених!", parse_mode="Markdown")
...
async def remove_user_start(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text("📝 Введіть Telegram ID користувача, якого потрібно видалити:")
...
async def process_remove_user(message: Message, state: FSMContext):
    ...
    try:
        ...
        if user_id not in current_users:
            await message.answer(f"⚠️ Користувач `{user_id}` не знайдений у списку.", parse_mode="Markdown")
            await state.clear()
            return
        ...
        await message.answer(f"✅ Користувач `{user_id}` успішно видалено зі списку!", parse_mode="Markdown")
